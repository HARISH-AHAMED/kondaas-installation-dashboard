                ...
                {(pincode || districtParam) ? (
                    ...
                    */
                    <div className="flex-1 w-full grid grid-cols-[20vw_49vw_26vw] gap-[1.2vw] px-[1.5vw] pt-[1.5vh] pb-[1.5vh] overflow-hidden">
                        ...
                        {/* RIGHT COLUMN */}
                        <div className="flex flex-col gap-[1.2vh] h-full overflow-hidden">
                            {/* Category Filter Tabs */}
                            <div className="grid grid-cols-3 gap-[0.8vw] shrink-0">
                                {['residential', 'commercial', 'waterHeater'].map(type => (
                                    <button 
                                        key={type} 
                                        onClick={() => setActiveTab(activeTab === type ? null : type)}
                                        className={`py-[1.2vh] rounded-[1.5vh] text-[1.4vh] 2xl:text-[1.6vh] font-bold transition-all shadow-sm border ${
                                            activeTab === type 
                                                ? 'bg-brand-red text-white border-brand-red shadow-red-500/10' 
                                                : 'bg-white text-slate-500 border-slate-100 hover:bg-slate-50'
                                        }`}
                                    >
                                        {type === 'waterHeater' ? 'Heater' : type.charAt(0).toUpperCase() + type.slice(1)}
                                    </button>
                                ))}
                            </div>

                            {/* Legacy Customers (Reformatted to white theme) */}
                            <div className="bg-white p-[1.8vh] rounded-[2vh] shadow-[0_0_20px_rgba(0,0,0,0.05)] border border-slate-100 shrink-0">
                                <div className="flex justify-between items-start mb-[0.6vh]">
                                    <h3 className="font-bold text-[1.6vh] text-slate-800">Legacy Customers</h3>
                                    <span className="text-emerald-500 text-[0.9vh] font-black uppercase tracking-widest">Verified Trust</span>
                                </div>
                                <div className="space-y-[0.5vh] mb-[0.8vh]">
                                    {[{ l: '3 Year Customers', c: 420 }, { l: '5 Year Customers', c: 285 }, { l: '10 Year Customers', c: 128 }].map(t => (
                                        <div key={t.l} className="flex justify-between items-center bg-slate-50/50 p-[0.8vh] rounded-xl border border-slate-100">
                                            <span className="font-bold text-slate-600 text-[1.3vh]">{t.l}</span>
                                            <span className="font-black text-slate-900 text-[1.5vh]">{t.c}</span>
                                        </div>
                                    ))}
                                </div>
                                <div className="bg-brand-red/5 p-[0.8vh] rounded-xl border border-brand-red/10">
                                    <div className="flex justify-between items-end mb-1">
                                        <span className="text-brand-red font-black text-[1vh] uppercase tracking-[0.05em]">ACTIVE RETENTION</span>
                                        <span className="font-black text-brand-red text-[1.8vh]">82%</span>
                                    </div>
                                    <div className="h-[0.35vh] bg-white rounded-full overflow-hidden"><div className="h-full bg-brand-red rounded-full" style={{ width: '82%' }}></div></div>
                                </div>
                            </div>
                            ...
                            <div className="bg-white p-[2vh] rounded-[2vh] shadow-[0_0_20px_rgba(0,0,0,0.05)] border border-slate-100 shrink-0">
                                <h3 className="font-bold text-[1.4vh] text-slate-400 uppercase mb-[1vh] tracking-widest">Service Result</h3>
                                <div className="flex flex-col gap-[1.2vh] py-[0.5vh]">
                                    <div className="flex items-center gap-[1vw]">
                                        <div className="bg-slate-50 p-[0.8vh] rounded-xl border border-slate-100 shrink-0 shadow-sm">
                                        ...
                                        <div><p className="text-[2.8vh] font-black text-slate-800 leading-none">1,248</p><p className="text-[1.1vh] font-bold text-slate-400 uppercase mt-0.5">Services Completed</p></div>
                                    </div>
                                    <div className="flex items-center gap-[1vw]">
                                        <div className="bg-slate-50 p-[0.8vh] rounded-xl border border-slate-100 shrink-0 shadow-sm">
                                        ...
                                        <div><p className="text-[2.8vh] font-black text-slate-800 leading-none">3.5 hrs</p><p className="text-[1.1vh] font-bold text-slate-400 uppercase mt-0.5">Avg Downtime</p></div>
                                    </div>
                                </div>
                            </div>

                            {/* Testimonial Showcase (Reduced padding to accommodate tabs) */}
                            <div className="bg-white p-[2vh] rounded-[2vh] shadow-[0_0_20px_rgba(0,0,0,0.05)] flex-1 flex flex-col justify-start gap-[1.5vh] border border-slate-100 relative overflow-hidden">
                                <div>
                                    <div className="flex justify-between items-center mb-[0.8vh]">
                                        <div className="flex items-center gap-[0.5vw]">
                                            <div className="w-[0.5vh] h-[2vh] bg-brand-red rounded-full"></div>
                                            <span className="text-slate-400 font-bold text-[1.1vh] uppercase tracking-[0.2em]">Testimony</span>
                                        </div>
                                        <span className="bg-brand-red/10 text-brand-red font-black text-[0.8vh] px-[0.8vh] py-[0.3vh] rounded-full uppercase tracking-widest border border-brand-red/20 shrink-0">
                                            {REVIEWS[activeReview].tenure}
                                        </span>
                                    </div>
                                    <p className="text-[1.4vh] font-medium text-slate-500 italic leading-relaxed line-clamp-3">"{REVIEWS[activeReview].text}"</p>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-[3.8vh] h-[3.8vh] rounded-full bg-brand-red flex items-center justify-center font-black text-white text-[1.5vh] shadow-lg shadow-red-500/20 shrink-0">{REVIEWS[activeReview].author[0]}</div>
                                    <div>
                                        <p className="font-black text-slate-800 text-[1.4vh] leading-none">{REVIEWS[activeReview].author}</p>
                                        <div className="flex gap-[0.1vw] mt-1">{[1, 2, 3, 4, 5].map(s => <svg key={s} className="w-[1vh] h-[1vh] text-amber-400" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>)}</div>
                                    </div>
                                </div>
                                <div className="absolute bottom-[2vh] right-[2vh] flex gap-[0.4vw]">
                                    {REVIEWS.map((_, i) => (
                                        <div key={i} className={`h-[0.5vh] rounded-full transition-all duration-300 ${i === activeReview ? 'w-[1.2vw] bg-brand-red' : 'w-[0.6vh] bg-slate-200'}`}></div>
                                    ))}
                                </div>
                            </div>
                        </div>
